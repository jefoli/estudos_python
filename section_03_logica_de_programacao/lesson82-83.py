...
Metódos úteis:
    -> (insert) - adiciona elementos em qualquer local da lista.
        ...
        Obs_2: Se passarmos um índice maior que o existente, ele vai inserir no último independente de existir ou não tal índice
    ...
    -> (extend) - extende uma lista.
        Obs_5: Não retorna valor, pois estende diretemante o valor a lista em que desejamos. Por conta disso, se criarmos uma nova var e extende-lá, seu retorno será NONE

    -> (+) - Através do polimorfismo, podemos fazer o sinal de (+) se comportar de outra maneira de modo a concatenar duas listas.
...
Obs_7: Em listas muitos grandes, merece maior atenção por conta da alteração dos índices em caso de CRUD.
'''
...
lista_d = lista_a.extend(lista_b) # não retorna valor, pois exetendeu os valores da lista_b na lista_a:
